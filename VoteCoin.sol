...
contract VoteCoin {
    ...
    function getUtxo(bytes32 _id) public view returns(address, uint, bytes32) {
    ...
    function getId(address _to, bytes32 _input) private view returns(bytes32) {
    ...
    function spend(bytes32 _id, uint _amount, address _to) public {
      ...
      utxos[newId1] = spend1;
      emit LogSpend(msg.sender, _to, oldUtxo.id, newId1, _amount);
      
      if (_amount < oldUtxo.value) {
        // slightly mutate the _id value to prevent a collision
        bytes32 newId2 = getId(msg.sender, _id ^ bytes32(uint256(1))); 
        UTXO memory spend2 = UTXO(msg.sender, oldUtxo.value - _amount, _id, newId2); 
        utxos[newId2] = spend2; 
        emit LogSpend(msg.sender, _to, oldUtxo.id, newId2, oldUtxo.value - _amount);
      }
    }
}